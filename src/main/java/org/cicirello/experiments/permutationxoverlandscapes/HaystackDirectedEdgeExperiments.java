...
/**
 * Experiments with the Permutation in a Haystack problem using Cyclic RType Distance.
 *
 ...
 */
public class HaystackDirectedEdgeExperiments {
    ...
    for (int seed = 1; seed <= NUM_INSTANCES; seed++) {
      PermutationInAHaystack problem =
          new PermutationInAHaystack(
              new CyclicRTypeDistance(), new Permutation(PERM_LENGTH, new SplittableRandom(seed)));
