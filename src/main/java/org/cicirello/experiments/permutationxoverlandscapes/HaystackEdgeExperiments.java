...
/**
 * Experiments with the Permutation in a Haystack problem using Cyclic Edge Distance.
 *
    ...
    for (int seed = 1; seed <= NUM_INSTANCES; seed++) {
      PermutationInAHaystack problem =
          new PermutationInAHaystack(
              new CyclicEdgeDistance(), new Permutation(PERM_LENGTH, new SplittableRandom(seed)));
